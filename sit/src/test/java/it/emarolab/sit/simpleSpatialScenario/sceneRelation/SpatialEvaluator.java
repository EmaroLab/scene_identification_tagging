package it.emarolab.sit.simpleSpatialScenario.sceneRelation;

import it.emarolab.amor.owlInterface.OWLReferences;
import it.emarolab.sit.simpleSpatialScenario.sceneElement.GeometricPrimitive;
import it.emarolab.sit.simpleSpatialScenario.sceneElement.Orientable;
...
public class SpatialEvaluator {
    ...
    private static final double SPATIAL_INTERVAL = .08; // in meters
    private static final double AXIS_INTERVAL = .3; // in meters
            ...
            for (GeometricPrimitive g2 : elements) {
                if (!g1.equals(g2)) {
                    ...
                    if ( g1 instanceof Orientable) {
                        evaluateCoaxial( (Orientable) g1, g2);
                    }
                    if ( g2 instanceof Orientable) {
                        evaluateCoaxial( (Orientable) g2, g1);
                    }*/
                }
            }
        }
    ...
    private void evaluatePerpendicular(Orientable domain, GeometricPrimitive range) { // TODO check parallel v.s. perpendicular and coaxial
        double dx = domain.getAxis().getX();
        double dy = domain.getAxis().getY();
        double dz = domain.getAxis().getZ();
